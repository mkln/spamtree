make_tree_old <- function(coords, na_which, axis_cell_size=c(5,5), K=c(2,2)){
...
make_J_bases <- function(coords, thresholds_list, p){
  n_thresh <- length(thresholds_list)
  
  Jlist <- list()
  for(i in 1:n_thresh){
    coords_res <- coords %>% as.matrix() %>% 
      spamtree:::axis_parallel(thresholds_list[[i]], 4) 
    Jlist[[i]] <- coords_res %$% block %>% spamtree:::Jmat(p)
  }

  return(Jlist)
}

make_tree <- function(coords, na_which, axis_cell_size=c(5,5), K=c(2,2), max_res=Inf){
  #Rcpp::sourceCpp("src/multires_utils.cpp")
