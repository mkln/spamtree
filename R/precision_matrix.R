#' @export
precision_matrix <- function(coords, theta,
  ...
  Ci <- spamtree:::spamtree_Cinv(cx, blocking, parents, block_names,
                      indexing, theta, Dmat, 1,
                      verbose, debug)
  
  original_order <- order(coords_blocking$ix)
  coords_blocking %<>% arrange(res, block, ix)
  csx <- coords_blocking$ix
  
  return(list(precision_matrix=Ci[original_order, original_order][csx,csx],
              coords_info = coords_blocking))
}