...
Building...")
  ...
  cat("Building reference set.\n")
  ptime <- system.time(
    mgp_tree <- spamtree:::make_tree_devel(coords, sort_mv_id, 
                                     axis_cell_size, K, max_res,
                                     add_nudge=rep(1e-10, dd))
  )
  #cat("Partitioning total time: ", as.numeric(ptime["elapsed"]), "\n")
  ...
  #na_blocks <- coords_blocking %>% arrange(Var1, Var2) %>% `[`(is.na(na_which),) %$% block %>% unique()
  #ixna <- simdata %>% filter(is.na(na_which)) %>% pull(ix)
  simdata %<>% right_join(coords_blocking %>% rename(mv_id = sort_mv_id) %>% dplyr::select(-ix))
  ...
  #  group_by(block) %>% 
  #  mutate(gix_block = as.numeric(factor(gix))) %>% as.data.frame()
  cx_all <- simdata %>% dplyr::select(!!!syms(coordsnames)) %>% as.matrix()
  gix_block   <- rep(1,nrow(simdata))#$gix_block
  ...
  blocking <- simdata$block
  
  block_info <- simdata %>% mutate(color = res) %>%
    dplyr::select(block, color) %>% unique()
