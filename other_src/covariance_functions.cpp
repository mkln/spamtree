...
void mvCovAG20107anis_inplace(arma::mat& res,
                          const arma::mat& coords, 
                          const arma::uvec& qv_block,
                          const arma::uvec& ind1, const arma::uvec& ind2, 
                          const arma::vec& ai1, const arma::vec& ai2, const arma::vec& phi_i, const arma::vec& thetamv, 
                          const arma::mat& Dmat, const arma::mat& S, bool same){
  int d = coords.n_cols;
  int p = Dmat.n_cols;
  if((d == 2) & (p < 2) & false){
    res = cexpcov(coords.rows(ind1), coords.rows(ind2), ai1(0), thetamv(0), same);
    ...
    if(same){
      for(int i=0; i<ind1.n_elem; i++){
        ...
        for(int j=i; j<ind2.n_elem; j++){
          delta = cxi - coords.row(ind2(j));
          xspace = delta.subvec(0, 1);
          h = pow(arma::conv_to<double>::from(xspace * S * xspace.t()), .5);
          u = d < 3? 0 : abs(delta(2));
...
arma::mat mvCovAG20107anis(const arma::mat& coords, const arma::uvec& qv_block, 
                       const arma::uvec& ind1, const arma::uvec& ind2, 
                       const arma::vec& ai1, const arma::vec& ai2, const arma::vec& phi_i, const arma::vec& thetamv, 
                       const arma::mat& Dmat, const arma::mat& S, bool same){
  ...
  int n2 = ind2.n_elem;
  arma::mat res = arma::zeros(n1, n2);
  mvCovAG20107anis_inplace(res, coords, qv_block, ind1, ind2, 
                       ai1, ai2, phi_i, thetamv, 
                       Dmat, S, same);
  return res;
}
...
void NonspatialUnivariate_inplace(arma::mat& res,
                                  const arma::mat& coords, const arma::uvec& ind1, const arma::uvec& ind2, 
                                  const double& sigmasq, const arma::vec& kweights, bool same){
  int d = coords.n_cols;
  if(same){
    for(int i=0; i<ind1.n_elem; i++){
      arma::rowvec cri = coords.row(ind1(i));
      for(int j=i; j<ind2.n_elem; j++){
        ...
        res(i, j) = sigmasq * exp(-weighted) + (weighted == 0? 1e-3 : 0);
      }
    }
    res = arma::symmatu(res);
  } else {
...
arma::mat NonspatialUnivariate(const arma::mat& coords, const arma::uvec& ind1, const arma::uvec& ind2, 
  ...
  int n2 = ind2.n_elem;
  arma::mat res = arma::zeros(n1, n2);
  NonspatialUnivariate_inplace(res, coords, ind1, ind2, sigmasq, kweights, same);
  return res;
}
